...
Every season my friends and I auction real cricketers and then accrue their actual fantasy points across the tournament — whoever assembled the best squad wins. The problem is the auction itself: with a spreadsheet, the early lots go for silly money and the marquee players you actually want sail past your remaining purse, because nobody has a feel for what a player is *worth* relative to the money in the room. I wanted a board that ran the auction live, tracked everyone's purse, and put a defensible price next to each name — so the table is arguing about strategy, not arithmetic. It's a personal-scale tool, but I built the valuation model the way I'd scope a real pricing feature: assumptions surfaced explicitly, every number traceable to data.
...
- **Prices every player before the hammer falls.** A valuation engine estimates an expected price per player from their recent fantasy-points form, so the room opens with a fair-value anchor instead of a blank slate.
